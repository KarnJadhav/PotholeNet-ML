...
Weight files (`*.pt`, `*.onnx`), the dataset's actual images/labels, and generated review folders (`qc_preview/`, `dedupe_review/`, `prediction_review/`, `attain_negatives/`, `runs/`) are gitignored. This repo tracks code and configuration only — weights live on Hugging Face Hub, and the dataset is reproducible from public/licensed sources via the scripts here.
